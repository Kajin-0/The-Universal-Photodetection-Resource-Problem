...
The distinction is elementary but consequential. A known deterministic delay contributes the phase factor $e^{-i\omega\tau}$ and changes latency without reducing distinguishability. Likewise, a known invertible deterministic linear filter that acts on both a signal and all noise generated upstream of the filter can attenuate amplitudes without reducing Fisher information. Information loss requires unresolved stochasticity, inaccessible or coarse-grained variables, downstream noise, finite observation resources, exact nulls, or an explicitly constrained temporal reference.
...
\label{eq:kernel}
\end{equation}
where $\tau\ge0$ is registration delay relative to photon arrival. The finite measure $\kappa$ has mass
\begin{equation}
\eta=\kappa(\M)\le1,
\end{equation}
and $\mu_m$ is a normalized conditional delay probability measure for $\kappa$-almost every mark. Translation covariance in time is part of the autonomy assumption: the kernel depends on elapsed delay, not on absolute source phase.
...
At zero frequency, $G(0)=\eta$. Retaining complete accessible marks is essential: marginalizing a mark that identifies a timing branch can create an artificial loss.
...
Thus purely non-atomic mark-conditioned delay laws force the flat-band average information transfer to vanish asymptotically. Deterministic or discrete timing branches leave an exact residue. A very narrow continuous prompt component has zero ultimate Wiener residue but can move the crossover to arbitrarily high frequency; a quantitative resource is therefore still needed.
...
Let $w(\omega)\ge0$ be a normalized incident spectral-information weight,
\begin{equation}
...
\label{eq:arbsource}
\end{equation}
Equation~\eqref{eq:flat} is recovered for uniform $w$ on $[-\Omega,\Omega]$.
...
\begin{proof}
For a normalized continuous delay law write $u(t)=\int_0^th_m(s)ds$, so $S_m=e^{-u}$, $f_m=h_me^{-u}$, and $u(t)\to\infty$. Then
\begin{equation}
\int_0^\infty f_m(t)^2dt
=\int_0^\infty h_m(u)e^{-2u}du
\le\frac{\Lambda(m)}{2}.
\end{equation}
Integration over $\kappa(dm)$ proves Eq.~\eqref{eq:R2H}.
\end{proof}
...
\section{Fixed mean and RMS jitter do not bound information bandwidth}
...
with
\begin{equation}
\lambda_\epsilon=\frac{\sqrt{\epsilon(2-\epsilon)}}{\sigma}.
\end{equation}
As $n\to\infty$, direct moments give
\begin{equation}
\operatorname{Var}(X_{\epsilon,n})\to\sigma^2,
\qquad
\mathbb E X_{\epsilon,n}\to
\mu_\epsilon\equiv\sigma\sqrt{\frac{\epsilon}{2-\epsilon}}.
\end{equation}
Fix any target mean $\mu_0>0$. For sufficiently small $\epsilon$ and sufficiently large $n$, define
\begin{equation}
D_{\epsilon,n}=X_{\epsilon,n}+\delta_{\epsilon,n},
\qquad
\delta_{\epsilon,n}=\mu_0-\mathbb E X_{\epsilon,n}\ge0.
\end{equation}
Then
\begin{equation}
\mathbb E D_{\epsilon,n}=\mu_0,
\qquad
\operatorname{Var}(D_{\epsilon,n})=\operatorname{Var}(X_{\epsilon,n})\to\sigma^2.
\end{equation}
The shift multiplies the characteristic function only by a phase, so its modulus is unchanged. For the unshifted mixture,
\begin{equation}
H_{\epsilon,n}(\omega)
=(1-\epsilon)\frac{n}{n+i\omega}
+\epsilon\frac{\lambda_\epsilon}{\lambda_\epsilon+i\omega}.
\end{equation}
On any prescribed finite frequency interval, first $n\to\infty$ and then $\epsilon\to0$ gives $|H_{\epsilon,n}(\omega)|\to1$ uniformly. Therefore fixed mean delay and fixed RMS jitter do not impose a finite upper information bandwidth.

This argument does not claim that a specified complete IRF or a specified prompt width is irrelevant. It shows that low-order moments alone do not control prompt concentration. In particular, an FWHM or similar width is not a substitute for the full timing law or for a microscopic rate resource unless additional shape assumptions are imposed.
...
Consider a restricted reversible Markov optical gateway
\begin{equation}
...
Suppose no primary electrical registration can occur before the first exit from state 1. The first gateway waiting time $T_1\sim\mathrm{Exp}(\lambda_1)$ is independent of exit destination and subsequent autonomous Markov dynamics. For every downstream mark $M$, write
\begin{equation}
D\mid M=T_1+Y_M,
\qquad Y_M\ge0.
\end{equation}
...
The absolute reverse optical rate $d$ in Eq.~\eqref{eq:Lstar} is a microscopic rate resource. It cannot in general be removed in favor of stationary thermodynamic quantities. Appendix~\ref{app:rare-fast} gives a reversible three-state family with fixed optical detailed-balance ratio, finite useful optical traffic, bounded stationary activity, and bounded entropy production, while its post-capture local rate and information bandwidth diverge. Thus
\begin{equation}
\boxed{
\text{stationary thermodynamic budgets alone do not determine the temporal information scale}.}
\end{equation}
...
A deterministic transit time is latency; unresolved transit dispersion can reduce information. An RC pole is an amplitude bandwidth and becomes an information limit only after the observation/noise model makes the filtered degrees of freedom irrecoverable. Low-order timing moments do not control prompt concentration. The resources in Eqs.~\eqref{eq:atomic}, \eqref{eq:R2}, and \eqref{eq:Hcap} target these distinct questions.

Talaga's TCSPC analysis already emphasized IRF-induced information loss, detector sensitivity--bandwidth tradeoffs, and the frequency-domain power spectrum of detector IRFs \cite{Talaga2009}. K\"ollner and Wolfrum quantified photon requirements for lifetime estimation \cite{KollnerWolfrum1992}; later FI analyses explicitly included finite IRFs and photon statistics \cite{Bouchet2019,TrinhEsposito2021}. Our parameter is instead a modulation of the incident source itself, and our claims concern the marked-event transfer theorem plus the atomic/collision/hazard resource hierarchy and its no-go/repair statements.
