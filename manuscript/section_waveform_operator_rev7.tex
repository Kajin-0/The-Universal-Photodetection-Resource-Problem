...
Let $E$ be a symmetric measurable frequency set. For a nonzero scalar perturbation $s$ with Fourier transform $S$ supported in $E$, define its source-normalized Fisher retention
\begin{equation}
\rho_G[s]
\equiv
...
\begin{proof}
Equation~\eqref{eq:RayleighG} is the Rayleigh quotient of the multiplication operator by $G$. The inequalities follow immediately because $|S|^2/\int|S|^2$ is a probability density on frequency. Let $g_-$ and $g_+$ denote the essential infimum and supremum on $E$. For every $\varepsilon>0$, the sets where $G<g_-+\varepsilon$ and $G>g_+-\varepsilon$ have positive measure whenever the corresponding essential bound is finite. Intersect either set with a sufficiently large symmetric finite interval and symmetrize it; because $G$ is even, this produces a symmetric finite-measure subset of positive measure. Taking $S$ to be the real even indicator of that subset gives $S\in L^1\cap L^2$, so its inverse Fourier transform is real, bounded, and square-integrable. The resulting Rayleigh quotient lies within $\varepsilon$ of the corresponding essential bound. Letting $\varepsilon\downarrow0$ proves sharpness.
\end{proof}
