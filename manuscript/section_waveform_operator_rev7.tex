...
Therefore preserving at least an absolute Fisher fraction $q$ for \emph{every} admissible weak waveform whose spectrum lies in $|\omega|\le\Omega$ is equivalent to the pointwise-almost-everywhere condition
\begin{equation}
G(\omega)\ge q
\qquad (|\omega|\le\Omega).
\label{eq:universalBandCondition}
\end{equation}
For square-integrable delay densities, Eq.~\eqref{eq:parseval} then implies
\begin{equation}
\pi\Rtwo
\ge\int_{-\Omega}^{\Omega}G(\omega)d\omega
\ge2\Omega q,
\end{equation}
so with $B=\Omega/(2\pi)$,
\begin{equation}
\boxed{\Rtwo\ge4Bq.}
\label{eq:universalBandCost}
\end{equation}
Together with $\Rtwo\le\Hcap$, the same coefficient gives
\begin{equation}
\boxed{\Hcap\ge4Bq.}
\end{equation}
Thus the Rev6 inverse resource cost is not only a flat-average necessary bound: it is also the necessary timing-resource cost of guaranteeing a minimum Fisher retention for every weak temporal waveform in a complete band-limited subspace.
