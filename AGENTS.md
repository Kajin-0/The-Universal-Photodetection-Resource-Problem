...
16. earlier WP0–WP24 notes only as needed.
...
# Strategic state
The mature core is the **autonomous proper marked-event detector** branch.

Working branch:

`agent/uprp-core-theorem-round10`

Scientifically verified manuscript base:

`manuscript/event_resource_theorem_rev3.tex`

Preferred next manuscript candidate is deterministic **Rev4**, generated by

`manuscript/apply_rev4.py`.

Rev4 adds WP34 plus two theorem figures without changing the underlying theorem class.

Do not reopen broad material-specific, analog-detector, non-Poisson, or quantum-pointer branches merely because they remain interesting.
...
## Atomic timing
If `p_j(m)` are atoms of the mark-conditioned delay measure,
\[
\boxed{
\lim_{\Omega\to\infty}
...
A global worst-case hazard is only a stronger corollary. Rare ultra-fast branches can be harmless if their capture weight vanishes sufficiently fast.
...
For a flat two-sided source-information task `|omega|<=Omega`, define
\[
B=\frac{\Omega}{2\pi}.
\]

To achieve absolute average source-normalized transfer
\[
\bar\eta_I\ge q,
\]
feasibility requires `q<=eta` and
\[
\boxed{
\mathfrak R_2\ge4Bq,
\qquad
...
If all markwise hazards obey a common ceiling `Lambda`,
\[
\boxed{
...
This is the operational inverse of the already-proved timing ceiling, not a new physical assumption.
...
Because finite-state Markov holding time in state 1 is exponential and independent of exit destination, any downstream mark generated only from exit destination and later autonomous Markov path satisfies
\[
...
Status: **scientifically audited base**. Its theorem constants, proof structure, citations, scope qualifications, and rare-fast appendix integration have been checked.

## Rev4 candidate
`manuscript/apply_rev4.py` generates `event_resource_theorem_rev4.tex` from Rev3.

It adds only:
- required TikZ/pgfplots packages;
- `figure_resource_hierarchy.tex`;
- WP34 minimum timing-resource corollary;
- `figure_jitter_no_go.tex`;
- one Discussion sentence highlighting `mathfrak H >= 4Bq`.

Generator Python syntax: VERIFIED.
Generator replacement anchors against Rev3: VERIFIED.

Figures:
- `manuscript/figure_resource_hierarchy.tex`
- `manuscript/figure_jitter_no_go.tex`
- `manuscript/fig_jitter_no_go_data.csv`

Both final figures have been locally compiled in a minimal RevTeX document and visually inspected. No overlap/clipping or overfull/underfull warnings remained after cleanup.

Full bibliography-resolved Rev4 GitHub Actions compile has **not** been observed through the current connector session. Do not claim full Rev4 compilation success.

Workflow:
`.github/workflows/manuscript-check.yml`

It safely generates Rev4, compiles it, and uploads the TeX/PDF artifact; it has no self-commit or issue-comment side effects.
...
> A resource-completeness theorem for source-modulation information transfer in autonomous marked photodetection event channels, with exact atomic and collision-intensity timing resources, an inverse timing-resource cost, and explicit no-go/repair results for low-order jitter moments, free synchronous control, and aggregate stationary thermodynamics.
...
1. Obtain an observable full Rev4 LaTeX compile result when possible; do not invent one.
2. Perform final line-by-line claim/reference audit of generated Rev4.
3. Keep only the two theorem figures unless another figure clearly adds information.
4. Decide whether to prepare a submission-ready source/package.
5. Defer non-Poisson/nonclassical source extension unless referee-style review shows it is necessary.
