# WP34 — Minimum Timing-Resource Cost Theorem
...
The total capture probability `eta` cancels from the **absolute capture-weighted timing-resource requirement**; `eta` remains in the feasibility condition `q<=eta`.
...
This has a direct operational interpretation: preserving an absolute fraction `q` of incident optical information across half-bandwidth `B` requires at least `4Bq` of capture-weighted timing-collision/local-registration capacity.
